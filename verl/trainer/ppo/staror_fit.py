...
from verl.trainer.staror_runtime.config import PipelineConfig
from verl.trainer.staror_runtime.dataset.loader import normalize_dataset_paths
from verl.trainer.staror_runtime.mcts.tree import FourStageMCTS
from verl.trainer.staror_runtime.pipeline.staror import StarORRunner
from verl.trainer.staror_runtime.reward.r3_batch_planner import (
...
from verl.trainer.staror_runtime.types import Generation, OptimizationTask, Stage, TrainingSample
...
def _flatten_stage_reports(stage_reports: dict[str, Any]) -> dict[str, float]:
    ...
    for key, value in stage_reports.items():
        ...
        if isinstance(reward_total, (int, float)):
            metrics[f"staror/{key}/best_reward"] = float(reward_total)
        stage_samples = value.get("stage_samples")
        if isinstance(stage_samples, (int, float)):
            metrics[f"staror/{key}/stage_samples"] = float(stage_samples)
    return metrics


class VerlRayPolicyBackend:
    """
    Adapter that lets the migrated StarOR MCTS code use verl actor/vLLM workers.
    """
    ...
    def __init__(self, trainer, pipeline_config: PipelineConfig):
        ...
        prior_temperature = _none_to_default(getattr(mcts_cfg, "prior_temperature", None), 0.7)
    ...
    def _adaptation_enabled(self) -> bool:
        return bool(getattr(self.pipeline_config.grpo, "enable_adaptation", True))
    ...
    def _reset_actor_episode_state(self) -> None:
        ...
        if getattr(self.trainer, "use_legacy_worker_impl", "disable") != "disable":
            print("[staror][WARN] sample-level LoRA reset is only implemented for the new worker path; skipping reset.")
            return
    ...
    def end_episode(self) -> None:
        try:
            ...
            print(f"[staror][WARN] actor LoRA sample reset failed: {type(exc).__name__}: {exc}")
        self._current_task_id = ""
    ...
    def generate(self, stage: Stage, prompt: Any, n: int, *, no_lora_adapter: bool = False) -> list[Generation]:
        ...
        effective_no_lora = bool(no_lora_adapter) or (not self._adaptation_enabled())
    ...
    def score_action_priors(self, stage: Stage, prompt: Any, candidates: list[str]) -> list[float]:
        ...
        try:
            ...
            print(f"[staror][WARN] prior fallback to uniform: {type(exc).__name__}: {exc}")
            n = len(candidates)
            return [1.0 / float(max(1, n))] * n
    ...
    def grpo_rollout_group(self, stage: Stage, prompt: Any, config, reward_callback):
        ...
        enable_adaptation = self._adaptation_enabled()
        no_lora_adapter = not enable_adaptation
        ...
        if len(completions) == 0:
            report = {
                ...
                "stage": stage.value,
                "num_samples": 0,
                "enable_adaptation": bool(enable_adaptation),
        ...
        if enable_adaptation and self._stage_update_enabled():
        ...
        elif not enable_adaptation and self._stage_update_enabled():
            stage_update_info = {
                "enabled": True,
                "applied": False,
                "reason": "enable_adaptation_false_no_grpo_update",
            }
        ...
        if not enable_adaptation:
            ...
            report = {
                ...
                "enable_adaptation": False,
                "reason": "enable_adaptation_disabled_no_grpo_no_lora",
    ...
    def prepare_task_context(self, task: OptimizationTask, dataset_config) -> dict[str, Any]:
        from verl.trainer.staror_runtime.mapping import build_mapping_extractor
    ...
    def generate_auxiliary_texts(
        ...
        *,
        ...
        return self._generate_messages(
            ...
            no_lora_adapter=not self._adaptation_enabled(),
        )
...
def build_pipeline_config_from_verl_config(config) -> PipelineConfig:
    pipe = PipelineConfig()
    section = config.algorithm.get("staror", {}) if config.algorithm is not None else {}
    ...
    pipe.log_dir = str(section.get("log_dir") or os.path.join(config.trainer.default_local_dir, "staror"))
...
def _resolve_raw_samples_for_path(trainer, pipeline_config: PipelineConfig, dataset_path: str):
    ...
    from verl.trainer.staror_runtime.dataset.loader import load_raw_task_dataset
...
def _prepare_r3_for_samples(raw_samples: list, runner: StarORRunner, rank: int, world_size: int) -> None:
    if not raw_samples or not runner.config.reward.enable_r3_reward:
        return

    cfg = runner.config
    print(f"[staror][r3] rank={rank}/{world_size} start precompute for {len(raw_samples)} samples")
    ...
    for idx, sample in enumerate(raw_samples):
        ...
        print(
            f"[staror][r3] [{idx + 1}/{len(raw_samples)}] sample_id={sample.sample_id} "
...
def run_staror_fit(trainer, logger) -> None:
    trainer.global_steps = 0
    trainer._load_checkpoint()
    pipeline_config = build_pipeline_config_from_verl_config(trainer.config)
    if bool(getattr(pipeline_config.grpo, "enable_adaptation", True)):
        trainer.checkpoint_manager.update_weights(trainer.global_steps)
    else:
        # In pure-MCTS mode (enable_adaptation=False), skip eager rollout weight sync.
        # vLLM is initialized from model path and does not require per-step policy syncing.
        print("[staror][info] enable_adaptation=false: skip initial checkpoint_manager.update_weights")
    ...
    runner = StarORRunner(backend=backend, config=pipeline_config)

    rank = int(os.environ.get("RANK", "0"))
    world_size = max(1, int(os.environ.get("WORLD_SIZE", "1")))
    dataset_paths = _resolve_dataset_paths_from_pipeline_config(pipeline_config)
    if not dataset_paths:
        print("[staror][dataset] no dataset paths configured, exiting.")
        return
    ...
    for dataset_path in dataset_paths:
        ...
        try:
            ...
            print(
                f"[staror][dataset][skip] failed to load dataset={dataset_path_str or dataset_path!r}; "
        ...
        print(
            f"[staror][dataset] rank={rank}/{world_size} total_samples={len(raw_samples)} "
    ...
    if not dataset_runs:
        print(
            f"[staror][dataset] no valid datasets loaded (paths={len(dataset_paths)}, "
            f"load_errors={skipped_dataset_load_errors}), exiting."
        )
        return

    if total_pending_samples <= 0:
        print("[staror][dataset] no pending samples found across all datasets, exiting.")
        return

    total_budget = max(1, total_pending_samples * max_iterations)
    if trainer.global_steps < 0:
        trainer.global_steps = 0
    progress_bar = tqdm(total=total_budget, initial=min(total_budget, max(0, trainer.global_steps)), desc="StarOR Progress")
    ...
    for dataset_idx, dataset_run in enumerate(dataset_runs, start=1):
        ...
        print(
            f"[staror][dataset-run] [{dataset_idx}/{len(dataset_runs)}] dataset={dataset_run['dataset_name']} "
        ...
        for sample_idx, sample in enumerate(ordered_samples):
            ...
            print(
                f"[staror][sample] [{sample_idx + 1}/{len(ordered_samples)}] sample_id={sample.sample_id} "
            ...
            metrics = {
                ...
                "training/epoch": 0.0,
                "staror/dataset_index": float(dataset_idx - 1),
                "staror/sample_index": float(sample_idx),
                "staror/sample_steps": float(steps_used),
                "staror/max_iterations": float(max_iterations),
                "staror/sample_best_reward": float(best_reward) if isinstance(best_reward, (int, float)) else float("nan"),
            }
            if best_obj is not None:
                try:
                    metrics["staror/sample_best_obj"] = float(best_obj)
                except Exception:
                    pass
            ...
            if isinstance(runtime.get("total_elapsed_sec"), (int, float)):
                metrics["staror/sample_runtime_sec"] = float(runtime["total_elapsed_sec"])
            metrics.update(_flatten_stage_reports(result.stage_reports))
            print(
                f"[staror][sample-done] sample_id={sample.sample_id} dataset={dataset_run['dataset_name']} gt={gold_answer} "
