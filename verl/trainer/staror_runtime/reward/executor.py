...
import re
from verl.trainer.staror_runtime.types import ExecutionResult, ModelInfo
...
def _install_gurobi_timelimit():
    raw = str(os.getenv("STAROR_GUROBI_TIMELIMIT_SEC", "") or "").strip()
    if not raw:
        return
    try:
        limit = float(raw)
    ...
    if optimize is None:
        return
    if bool(getattr(optimize, "_staror_time_limit_wrapped", False)):
        return
    ...
    _wrapped_optimize._staror_time_limit_wrapped = True
...
class PythonCodeExecutor:
    ...
    def _init_sandbox(self) -> None:
        sandbox_path = Path(tempfile.mkdtemp(prefix="staror_sandbox_"))
    ...
    def _run_in_subprocess_mode(self, code: str, instance: dict[str, Any]) -> ExecutionResult:
        start = time.perf_counter()
        with tempfile.TemporaryDirectory(prefix="staror_") as td:
            temp_dir = Path(td)
        ...
        for idx, line in enumerate(lines):
            ...
            if match is not None:
                ...
                if not write_re.match(previous):
                    ...
                    new_lines.append(f'{indent}{var_name}.write("staror_model.lp")')
                    inserted = True
    ...
    def _invoke_runner(
        ...
        if isinstance(self.gurobi_time_limit_sec, (int, float)) and float(self.gurobi_time_limit_sec) > 0.0:
            env["STAROR_GUROBI_TIMELIMIT_SEC"] = str(float(self.gurobi_time_limit_sec))
