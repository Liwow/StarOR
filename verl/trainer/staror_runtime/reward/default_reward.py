...
from verl.trainer.staror_runtime.config import RewardConfig
from verl.trainer.staror_runtime.model.backend import PolicyBackend
from verl.trainer.staror_runtime.reward.base import RewardCalculator
from verl.trainer.staror_runtime.reward.clusters import SemanticCluster, StructuralCluster
from verl.trainer.staror_runtime.reward.executor import PythonCodeExecutor
from verl.trainer.staror_runtime.reward.perturbation import generate_perturbed_instances_from_map
from verl.trainer.staror_runtime.types import ExecutionResult, OptimizationTask, RewardBreakdown, Stage, Trajectory


@dataclass(slots=True)
class StarORRewardCalculator(RewardCalculator):
    ...
    def _resolve_reward_weight_plan(self, trajectory: Trajectory, current_iter: int) -> dict[str, Any]:
        default_early = (0.2, 0.5, 0.2, 0.1)
        default_mid = (0.4, 0.4, 0.1, 0.1)
    ...
    def _prepare_structural_group(evals: list[dict[str, Any]]) -> dict[tuple[int, int, int, int, int, int], int]:
        ...
        for item in evals:
            ...
            normalized = StarORRewardCalculator._normalize_structural_feature_tuple(feature_tuple)
    ...
    def _extract_objective_numeric(output: object | None) -> float | None:
        ...
        if isinstance(output, dict):
            ...
            for wrapper_key in ("result", "value", "val", "data"):
                ...
                if nested is not None:
                    numeric = StarORRewardCalculator._extract_objective_numeric(nested)
                    if numeric is not None:
                        return numeric

            repr_value = output.get("repr")
            if repr_value is not None:
                numeric = StarORRewardCalculator._extract_objective_from_text(str(repr_value))
                if numeric is not None:
                    return numeric
            ...
            for _, value in output.items():
                if isinstance(value, (dict, list, tuple)):
                    numeric = StarORRewardCalculator._extract_objective_numeric(value)
                    if numeric is not None:
                        return numeric

        if isinstance(output, (list, tuple)):
            for item in output:
                numeric = StarORRewardCalculator._extract_objective_numeric(item)
                if numeric is not None:
                    return numeric
        ...
        if isinstance(output, str):
            try:
                ...
                return StarORRewardCalculator._extract_objective_from_text(output)
            numeric = StarORRewardCalculator._extract_objective_numeric(maybe)
            if numeric is not None:
                return numeric
            return StarORRewardCalculator._extract_objective_from_text(output)
    ...
    def _execution_summary(
        ...
        obj_hit = StarORRewardCalculator._is_valid_objective(obj_answer)
        ...
        return {
            ...
            "lp_injection_applied": bool(getattr(execution, "lp_injection_applied", False)),
            "output": StarORRewardCalculator._jsonable(execution.output),
            "stdout_tail": StarORRewardCalculator._truncate_text(execution.stdout, 2000),
            "stderr_tail": StarORRewardCalculator._truncate_text(execution.stderr, 2000),
        }
