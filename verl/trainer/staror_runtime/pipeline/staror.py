...
from verl.trainer.staror_runtime.mcts import FourStageMCTS
from verl.trainer.staror_runtime.model.backend import PolicyBackend
from verl.trainer.staror_runtime.prompts import PromptBuilder, get_prompt_profile
from verl.trainer.staror_runtime.reward import StarORRewardCalculator
from verl.trainer.staror_runtime.types import OptimizationTask, RunTrace, Stage, StageTrace, Trajectory
...
class StarORRunner:
    ...
    def run_task(self, task: OptimizationTask, human_gold_answer: str | None = None) -> TaskRunResult:
        ...
        try:
            rewarder = StarORRewardCalculator(task=task, backend=self.backend, config=self.config.reward)
            mcts = FourStageMCTS(
