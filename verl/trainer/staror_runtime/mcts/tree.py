...
from verl.trainer.staror_runtime.config import GRPOConfig, MCTSConfig
from verl.trainer.staror_runtime.mcts.node import SearchNode
from verl.trainer.staror_runtime.mcts.puct import PUCTSelector
from verl.trainer.staror_runtime.prompts import PromptBuilder
from verl.trainer.staror_runtime.prompts.refine_code import (
    CODE_ERROR_PROMPT_TEMPLATE,
    CODE_INFEASIBLE_PROMPT_TEMPLATE,
    CODE_REFINE_PROMPT_TEMPLATE,
)
from verl.trainer.staror_runtime.types import Generation, OptimizationTask, RewardBreakdown, STAGE_ORDER, Stage, Trajectory
...
class FourStageMCTS:
    ...
    def _code_entry_suppress_weight(self) -> float:
        raw = getattr(self.config, "code_entry_same_cluster_suppress_weight", 0.5)
        try:
            return max(0.0, min(1.0, float(raw)))
        except Exception:
            return 0.7
    ...
    def _semantic_rel_tol(self) -> float:
        ...
        tol = getattr(reward_cfg, "global_consensus_rel_tol", 0.0001)
        try:
            return float(tol)
        except Exception:
            return 0.005
