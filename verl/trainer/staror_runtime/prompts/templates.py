...
from verl.trainer.staror_runtime.types import DEFAULT_STAGE_ORDER, SOLVERLLM_STAGE_ORDER, Stage
...
IMPORTANT: Do not skip any steps. AFTER the TYPE and SET sections are fully completed, you must output the full modeling and solving Python code inside <python> without other explanations. Proceed with the task now.
""".strip(),
        Stage.SET_PARAM_VAR: f"""
You are a professional optimization problem analyst, proficient in extracting key elements from optimization problems described in natural language.
...
IMPORTANT: Do not skip any steps. AFTER the Parameters and Variables sections are fully completed, you must output the full modeling and solving Python code inside <python> without other explanations. Proceed with the task now.
""".strip(),
        Stage.OBJ_CONS: f"""
You are a professional optimization problem analyst, proficient in extracting key elements from optimization problems described in natural language.
...
IMPORTANT: Do not skip any steps. AFTER the Objective and Constraints sections are fully completed, you must output the full modeling and solving Python code inside <python> without other explanations. Proceed with the task now.
""".strip(),
        Stage.CODE: f"""
You are an optimization expert. You should solve the optimization problem and only Provide the corresponding Gurobi Python code to implement the model within <python> and </python>
