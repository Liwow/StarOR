...
class MCTSConfig:
    ...
    code_entry_same_cluster_suppress_weight: float = 0.5
...
class RewardConfig:
    ...
    global_consensus_rel_tol: float = 0.0001
    ...
    dynamic_reward: bool = True  # Enable dynamic reward schedule by MCTS iteration / code-entry
    # Dynamic schedule weight vector for early phase (iter<=3), as "r1,r2,r3,r4".
    early_weight: str = "0.2,0.5,0.2,0.1"
    # Dynamic schedule weight vector for middle phase (4<=iter<=5), as "r1,r2,r3,r4".
    mid_weight: str = "0.4,0.4,0.1,0.1"
    ...
    r1_obj_scale_fail_multiplier: float = 0.5  # Multiplier on r1 weight when obj-scale check fails
    r3_weight: float = 0.2  # Weight for r3 in final reward
    r4_weight: float = 0.1  # Weight for r4 in final reward
    structure_gate_min: float = 1.0  # Minimum multiplier when LP structure is incomplete


@dataclass(slots=True)
class GRPOConfig:
    # Master switch for instance-level test-time adaptation.
    ...
    stage_update: bool = True
    learning_rate: float = 1e-4
    group_size: int = 8  # Alias of num_generations in GRPO literature.
