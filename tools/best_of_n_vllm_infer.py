    ...
    debug_clusters = [
        {
            ...
            "members": [float(v) for _, v in c["members"]],
        }
        for c in clusters
    ]
    return voted, int(len(winner["members"])), debug_clusters
    ...
    *,
    ...
    for idx, text in enumerate(outputs):
        ...
        if isinstance(obj, (int, float)) and math.isfinite(float(obj)):
            candidate_objs.append(float(obj))
        candidates_payload.append(
            {
                "candidate_index": int(idx),
                "obj": (float(obj) if isinstance(obj, (int, float)) and math.isfinite(float(obj)) else None),
                "text": (text if bool(args.save_raw_text) else text[:400]),
            }
        )

    voted_obj, vote_count, vote_clusters = cluster_vote(
        candidate_objs,
        rel_tol=float(args.vote_rel_tol),
        abs_tol=float(args.vote_abs_tol),
    )
    ...
    if voted_obj is not None and gt_numeric is not None:
        abs_err = abs(float(voted_obj) - float(gt_numeric))
        rel_err = abs_err / max(abs(float(gt_numeric)), 1.0)
        hit = bool(is_close(float(voted_obj), float(gt_numeric), rel_tol=float(args.gt_rel_tol), abs_tol=float(args.gt_abs_tol)))
...
def build_summary(log_path: Path) -> dict[str, Any]:
    ...
    for _, row in iter_jsonl(log_path):
        ...
        voted = row.get("voted_obj")
        gt = row.get("gt_obj")
        if isinstance(voted, (int, float)) and math.isfinite(float(voted)):
            with_vote += 1
        if isinstance(gt, (int, float)) and math.isfinite(float(gt)):
            with_gt += 1
        if isinstance(voted, (int, float)) and isinstance(gt, (int, float)):
            if math.isfinite(float(voted)) and math.isfinite(float(gt)):
                comparable += 1
                ae = abs(float(voted) - float(gt))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Best-of-N inference via local vLLM OpenAI-compatible API.")
    ...
    parser.add_argument("--n", type=int, default=8, help="Number of candidates per sample.")
    ...
    parser.add_argument("--vote-rel-tol", type=float, default=1e-4)
    ...
    parser.add_argument("--save-raw-text", action="store_true", help="Save full candidate text in logs.")
    ...
    log_path = out_dir / f"bestofn_{dataset_tag}__{model_tag}.jsonl"
    summary_path = out_dir / f"bestofn_{dataset_tag}__{model_tag}.summary.json"
    ...
    print(
        f"[bestofn] input={input_path} model={args.model} n={args.n} parallel={args.parallel} "
        f"tasks={len(tasks)} log={log_path}",
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max(1, int(args.parallel))) as pool:
        ...
        for fut in as_completed(fut_map):
            ...
            if processed % 10 == 0 or processed == len(tasks):
                print(f"[bestofn] processed={processed}/{len(tasks)}", flush=True)
    ...
    print(f"[bestofn] done log={log_path} summary={summary_path}", flush=True)
    print(f"[bestofn] comparable={summary.get('comparable')} hit_rate={summary.get('hit_rate')} mae={summary.get('mae')}", flush=True)
    return 0
